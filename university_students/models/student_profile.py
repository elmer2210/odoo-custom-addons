...
class StudentProfile(models.Model):
    ...
    completename = fields.Char(string='Nombres Completo',  compute='_compute_completename', store=True)
    email = fields.Char(string='Correo Electrónico', required=False)
    campus_id = fields.Many2one('university_management.campus', string='Sede', required=True)
    faculty_id = fields.Many2one('university_management.faculty', string='Facultad', required=True)
    career_id = fields.Many2one('university_management.career', string='Carrera', required=True)
